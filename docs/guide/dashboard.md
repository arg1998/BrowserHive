...
Every session, live and finished. Filter by state (live, closed, archived), owner, channel and persistence mode; search by slug or id; sort by any column. Columns show the last URL, calls and errors, blocked attempts, the lease countdown and the state (including "needs attention" and "being watched"). Select rows to archive, unarchive or delete in bulk. Deleting names exactly what is erased: events, trace, screenshots, profile and downloads, terminating live sessions first.

## Session detail

The header shows the session id, state and actions: terminate, open the trace viewer, download `trace.zip`, archive, delete. When the agent is waiting on you, a banner shows the attention request or pending vault confirmations. A side rail lists owner, channel, headless or headed, persistence, current URL, lease and identity.
