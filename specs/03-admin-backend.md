...
| GET | `/system/mcp/connections` | S | `limit` 1..200 (default 50), `offset` ≥ 0 (default 0) | `{connections:[McpConnectionRow], live, total, now}` — live connections first (most recently seen first), then recent closed ones; skips `offset` rows of that order and returns up to `limit`; `live` counts open rows and `total` every stored row, so a client pages with `offset` += `limit` while `offset < total`. `McpConnectionRow` = `connection_id, transport, principal, live, harness, harness_label, harness_source, model, model_source, workspace, client_name, client_version, client_title, protocol_version, user_agent, ip, connected_at, last_seen_at, closed_at, sessions, conflicts:[{source, value, harness}], meta:{…}` (02 §1.4; rows written before schema v3 read their harness from the stored header value, else `unknown`) |
