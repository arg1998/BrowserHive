...
| 4 | `hostGuard` | Host allow-list: loopback names, the configured `--host`, plus `--allowedHosts` entries. Mismatch → 421 `HOST_NOT_ALLOWED`. This is the DNS-rebinding defense; the MCP transport's own `allowedHosts` option is fed the same list. |
...
Commands (`name` → params → scope): `screencast.start {session_id, max_width?, max_height?, quality?}` → `sessions:read`; `screencast.stop {session_id}`; `screencast.set_size {session_id, max_width, max_height}`; `input {session_id, input: LiveInput}` → `sessions:takeover` (gated on an open attention request **per message**; refusal is `error INPUT_NOT_PERMITTED`, the socket stays up; every accepted input is audited as `operator.input` events, coalesced per second per session); `session.set_viewport {session_id, width, height}` → `sessions:write` (not attention-gated, D-10); `logs.tail {level?, module?}` → `logs:read` (alias for subscribing `logs` with a filter; `level` is a **minimum** level and `module` a root prefix, whereas REST `level` is an exact set).
...
- `mcp_connections` records what the transport and `initialize` reveal (IP, headers, `clientInfo`, protocol version, capabilities, `_meta`). An explicit client-metadata tool is not added, because the tool catalog is a stable contract (D-12) and self-reported metadata is dashboard-only anyway.
