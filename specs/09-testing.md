...
- `config-schema.json` — the config JSON Schema. **Invariant:** every key has `description` and `x-browserhive-env`; reserved keys are listed with `x-browserhive-reserved: true`; every property accepts a string holding an `{env:NAME}` reference, and every enum still rejects a string that is neither a member nor a reference.
...
- Harness identity (D-30): `contracts/test/harness.test.ts` is table-driven over the slug/label/alias tables (every known `clientInfo.name` and `User-Agent` pattern, generic SDK names → `unknown`, declared spellings `Claude Code`/`claude_code` → `claude-code`, sanitising of unrecognised values, `metricHarness` folding, the meta-bag caps at 16 keys / 256 B / 4 KiB with the dropped count); `interface/mcp/identity.test.ts` covers the resolution ladder row by row (each source beating every lower one, `browserhive.ai/` read as an alias, per-call `_meta` over `initialize`, per-request headers over the `initialize` request's), conflicts recorded and logged once per connection, the connection row updated only when the resolution changes, and model/workspace sources. The HTTP transport test drives a real SDK client with headers, `?harness=`, `_meta` and `User-Agent`; the stdio listener test covers `BROWSERHIVE_HARNESS` and `CLAUDECODE`; the config suite pins that the identity names are skipped and that `BROWSERHIVE_HARNES` suggests `BROWSERHIVE_HARNESS`. **Invariant:** no identity value changes a tool's result or an authorization decision (the dispatcher suite runs the same call with and without identity signals and compares the shaped results).
...
- Browser detection (`infra/browsers/browser-detection.test.ts`): per-OS fake filesystems, `--version` output and registries (Linux `/etc/opt/*/policies/managed/*.json`, macOS managed preferences through `plutil`, Windows `reg query` for HKLM and HKCU, the Windows version-named directory); a path Playwright names but that is gone counts as not installed; nothing is run for a missing browser; a test pins that Playwright's internal `registry.findExecutable` still resolves the bundled browser, so a Playwright bump that moves it fails loudly (D-26).
- Sandbox facts (`infra/browsers/sandbox.test.ts`): the Ubuntu, root and Docker launch failures are recognised and other failures are not; Chrome's own sentence is extracted, never guessed; host conditions per OS (the AppArmor sysctl, `unprivileged_userns_clone`, `max_user_namespaces`, root, container markers) with fake files; profile coverage; the printed AppArmor profile; the two-launch probe (`works` / `unavailable` only when the unsandboxed launch works / `broken` / `not-installed`).
- Sandbox guidance (`infra/browsers/sandbox-guidance.test.ts`): per-OS fixtures, including the exact refusal block for Ubuntu 24.04 with Chrome installed; no working browser (Chrome install and AppArmor offered); root in Docker (root/container advice, never AppArmor); `unprivileged_userns_clone=0`; macOS/Windows (report request, no Linux advice); a covered path; a branded browser without a profile; the agent-facing short form.
- Sandbox policy (`infra/browsers/sandbox-policy.test.ts`): the fallback matrix at launch time with fake attempts: `off` never sandboxes; `auto` falls back once and warns once, keeps a working browser sandboxed, serialises concurrent first launches, keeps verdicts per executable, blames an unrecognised failure only because the browser starts without the sandbox, surfaces a browser broken either way, passes typed errors through, never attempts the sandbox as root; `on` and an agent's `chromiumSandbox: true` fail typed and then fail fast from the cache; a required sandbox with an unrecognised failure is confirmed by a proof launch that is disposed. `playwright-browser-driver.test.ts`: a sandbox launch failure is `SANDBOX_UNAVAILABLE` with `retryable: never` (it was `INTERNAL_ERROR`/`backoff`; the test was run on the old code first).
...
- `doctor`: each check with injected probes; exit 0/1/2; `--json` shape; the browser rows (a missing configured channel fails, version drift warns only for the channel in use, blocking policies fail for the configured channel), the sandbox verdict judged per mode, root/container, the guidance block (working browser first, AppArmor next), `--printApparmorProfile` for installed and missing browsers.
- `init`: idempotent re-run reports "already installed"; failure of the browser download is reported and exits 1 with the exact command to retry; the detection report; no prompt without a terminal, in CI or in a container; Enter keeps the current browser and writes nothing; a pick asks to save and creates the data-dir config file (0600) or merges into the existing one (other keys, order and `$schema` kept); `--channel` without `--yes` and without a terminal fails and writes nothing; a missing channel changes nothing; `--installChrome` runs `playwright install chrome` and names the retry on failure; an overriding env var is pointed out; the menu's pros and cons are computed from the probes.
- Composition sandbox preflight (`test/composition/sandbox.test.ts`, fake probes): `sandbox=on` refuses with `SANDBOX_UNAVAILABLE`, the guidance lists a browser that was checked to work first, only the failing browser's alternatives are probed, and the data dir is released; a working browser boots after one probe; a missing configured browser refuses with the install command; `auto`/`off` probe nothing at boot.
- `config show/validate/schema`: provenance table (`config-file via $VAR`, a secret key's reference named but its value never printed); JSON output validated against the schema, including `refs` and `template`; a config file with a reference on every key validates against the published JSON Schema.
...
- `sandbox-stealth.test.ts` (`packages/browserhive/test/integration/`) — through BrowserHive (`createServer` → MCP → `launch_session` → `navigate` → `evaluate`) for every installed channel: `sandbox=auto` never fails a launch, `off` stays unsandboxed, every launched session passes the page-visible stealth checks (no `HeadlessChrome`, `Google Chrome` exactly once, `webdriver === false`, `window.chrome`, 5 plugins, `pdfViewerEnabled`, H.264/AAC, `fullVersionList` equal to the engine's real version), sandboxed and unsandboxed sessions report identical signals, and a required sandbox either works or fails as `SANDBOX_UNAVAILABLE`, `retryable: never`. Channels not installed skip with the reason printed; the bundled Chromium is never skipped. Sandbox state is read from the browser processes' command lines. The weekly `real-chrome` job runs the whole suite with `BHDEV_TEST_CHANNEL=chrome` (sessions on the installed Chrome, 06 §6.1).
