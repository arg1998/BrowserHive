...
- `auto` runs each browser sandboxed where it can. The first real launch of each executable tries the sandbox; if that fails and the same browser then starts without it, the sandbox is the cause: the verdict is cached for the process lifetime and sessions fall back with one `warn` log (`sandbox fell back`), a `/system` line and a `doctor` warning. It never fails a launch because of the sandbox, never retries it per session, and does not attempt it as root.
