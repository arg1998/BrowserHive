...
- **Start** after the stage has been measured (or 300 ms have passed): `command('screencast.start', { session_id, max_width, max_height, quality })`. `max_*` for `fit` = stage box × `min(2, max(1, devicePixelRatio))`, clamped to 64…`SCREENCAST_MAX_DIMENSION`; presets 720p = 1280×720, 1080p = 1920×1080, native = 3840×2160. Quality 80 in the pane, 85 in fullscreen.
...
- **No links to a documentation site.** Error codes, the blocklist and the vault explain themselves inline (§11 Copy); `EmptyState.docsHref` exists for when published documentation exists. The sessions zero-data CTA links to the repository README.
