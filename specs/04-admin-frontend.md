...
- **Panels** (12-column from `xl`: left 7 = Recent sessions, Websites visited; right 5 = Recent failures, Most visited domains; stacked below): flush `LinkList` rows that are whole-row links, each with a `PanelLink` header action.
  ...
  - **Harnesses** (right column, under Most visited domains; `GET /metrics/harnesses` over the page window): one whole-row link per harness to `/sessions?harness=<slug>` keeping `since`/`until`: label (the known label, else the slug), `N sessions` (with `· N live` when > 0) and `N calls` (with errors in red when > 0), and a thin meter of its share of sessions. `Unknown` is always listed, last, muted. The header carries the self-reported `InfoDot` (§11) and a `PanelLink` to `/sessions`. Empty window: a compact zero-data line.
...
Search: table schema + `view` (`live|closed|archived`, absent = all non-archived), `owner`, `channel` (csv), `persistence` (csv), `harness` (csv of slugs), `since`, `until`, `archived` (`include`). Sort keys: `created|slug|channel|activity|errors|lease|owner|persistence|closed|blocked|harness`. Selection is local state keyed to the filter signature and never in the URL (§1 principle 1).

- **Header**: "N sessions · N live" only when unfiltered; the explainer is in `learnMore`. "N matching" shows only when filtered.
- **Filters** inside `SelectionToolbar`: view toggle, Owner select and facet chips (Channel, Persistence, **Harness**) rendered only when they have more than one option or a selection, search, time range, View menu. Harness chips show the label (`Claude Code`, `Unknown`) with the facet count.
- **Columns**: Session (slug `font-medium`, underlines on row hover; second line = unique id suffix in mono with the full id in the tooltip and copy, `· chrome/headed/incognito` only when not default, owner only when facets show more than one) · State (dot + text; second line = lease bar while live, "lease paused" during attention, or relative closed time) · Last URL (`grow`, host emphasised) · Activity (`12 calls`, `2 errors · 1 blocked` in red only when > 0; `sortAliases` errors/blocked) · **Agent** (sortable `harness`; the harness label, `Unknown` as a muted chip, never an empty cell; second line the workspace when reported, else the model when reported) · Created (relative, absolute tooltip) · actions (`revealOnHover`). Rows ≈ 53 px, two lines. Columns follow container queries (§10); Agent shows from 48rem of table width and otherwise folds into the Session line as `· Claude Code` (omitted there when `unknown`).
...
- **Cards** under 768: whole-card links with slug, id, host+path, state/lease, activity and the harness label (`Unknown` muted).
...
- **Activity** (`activity/*`): one stream of tool calls, page visits, attention requests, vault fills and blocked navigations, so the operator reads a session in one place instead of cross-referencing per-kind lists:
  ...
  - Row detail (inline expand, click anywhere on the row head or the chevron `button[aria-expanded]`): tool calls fetch `…/tool-calls/:event_id` and show facts (including the call's Harness), error box, "Landed on", Parameters and Result `JsonView` (Result hidden when it only echoes the error), screenshot with zoom; other kinds show their full record; **Copy as JSON** everywhere.
...
- **Configuration**: settings in effect as grouped key/value panels in two independent stacks (Server + Integrations beside Sessions + Stealth, `items-start`); mono only for Bind address and Data directory; paths wrap at `/`. "All configuration keys" table: key, wrapping value, source chip, "overrides env 9000" lines ("overrides file … via $VAR" when the shadowed value used references), filter + "only changed from defaults" + "only values from references". A value that came through config-file references (08 §3.1) shows one mono chip per variable (`$OTLP_HOST`, "default" marked) under the source chip; the chip is a button whose popover names the variable and, for keys that are not secret, shows the value as written in the file (`http://{env:OTLP_HOST}:4318`); a secret key keeps its `redacted` chip and still shows the variable. The filter also matches variable names. The panel's info popover explains references and links to the configuration guide's References section.
