...
Decisions: D-13 (stealth posture, proxy seam), D-21 (creation pipeline), D-22 (blocklist reload), D-20 (privacy), D-25 (not built).
...
- `deriveCoherentIdentity()` (pure, sync):
  ...
  - UA-CH `brands` = `[GREASE 'Not_A Brand'/24, Chromium/<major>, Google Chrome/<major>]`; `fullVersionList` likewise with the full version. Adding `Google Chrome` is the point (bundled Chromium reports only Chromium + GREASE).
...
| `defaultChannel` | `chromium \| chrome \| edge` | `chromium` | per-session `channel` overrides |
